...
const CONCEPTS = [
  ...
  },
  {
    "type": "INGRESOS",
    "category": "Ingresos",
    "concept": "Alquiler Macro",
    "row": 7
  },
  {
    "type": "INGRESOS",
    "category": "Ingresos",
    "concept": "Alquiler Dpto Cba",
    "row": 8
  },
  {
    "type": "INGRESOS",
    "category": "Ingresos",
    "concept": "Retiro Sociedad",
    "row": 9
  },
  {
    "type": "INGRESOS",
    "category": "Ingresos",
    "concept": "Intereses FCI",
    "row": 10
  },
  {
    "type": "INGRESOS",
...
createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/app-icon.svg') return sendTextFile(res, 'image/svg+xml; charset=utf-8', APP_ICON_SVG);
...
async function createExpense(body) {
  ...
  const shouldAddToPreviousValue = await shouldAccumulateExpense(month.column, concept.row);
  const newValue = shouldAddToPreviousValue ? previousValue + amount : amount;

  await updateValues(targetRange, [[newValue]]);
  await paintExpenseCell(month.column, concept.row);
...
async function shouldAccumulateExpense(column, row) {
  const rowNumber = Number(row);
  if (!ACCUMULATING_ROWS.has(rowNumber)) return false;

  try {
    const rows = await getValues("'" + LOG_SHEET_NAME + "'!A2:O", 'UNFORMATTED_VALUE');
    return rows.some((item) =>
      String(item[7] || '').trim() === String(column).trim()
      && Number(item[8] || 0) === rowNumber
      && String(item[14] || '').trim().toLowerCase() === 'cargado'
    );
  } catch (error) {
    if (String(error.message || '').includes('Unable to parse range')) return false;
    throw error;
  }
}
...
async function paintExpenseCell(column, row) {
  const metadata = await getSpreadsheetMetadata();
  const sheet = metadata.sheets?.find((item) => item.properties?.title === MAIN_SHEET_NAME);
  if (!sheet?.properties?.sheetId && sheet?.properties?.sheetId !== 0) throw new Error('No encontre la hoja principal para pintar la celda');
...
async function getSpreadsheetMetadata() {
  const accessToken = await getAccessToken();
  const response = await fetch('https://sheets.googleapis.com/v4/spreadsheets/' + spreadsheetId + '?fields=sheets.properties(sheetId,title)', { headers: { authorization: 'Bearer ' + accessToken } });
  const data = await response.json();
  if (!response.ok) throw new Error(JSON.stringify(data));
  return data;
}
...
function contentType(filePath) {
  return { '.html': 'text/html; charset=utf-8', '.css': 'text/css; charset=utf-8', '.js': 'text/javascript; charset=utf-8', '.png': 'image/png', '.pdf': 'application/pdf', '.webmanifest': 'application/manifest+json; charset=utf-8' }[extname(filePath)] || 'application/octet-stream';
}
...
function parseAmount(value) {
  ...
  const decimals = separatorIndex >= 0 ? raw.slice(separatorIndex + 1).replace(/\D/g, '') : '';
  const integerPart = separatorIndex >= 0 ? raw.slice(0, separatorIndex) : raw;
  const separatorIsDecimal = decimals.length > 0 && decimals.length <= 2;
  const normalized = separatorIsDecimal
    ? integerPart.replace(/\D/g, '') + '.' + decimals
    : raw.replace(/\D/g, '');
...
function columnToIndex(column) {
  return String(column).toUpperCase().split('').reduce((index, letter) => index * 26 + letter.charCodeAt(0) - 64, 0) - 1;
}

function sendBase64File(res, contentTypeValue, base64Value) {
  const fileBuffer = Buffer.from(base64Value, 'base64');
  res.writeHead(200, { 'content-type': contentTypeValue, 'content-length': fileBuffer.length, 'cache-control': 'public, max-age=86400' });
  res.end(fileBuffer);
}

function sendTextFile(res, contentTypeValue, value) {
  res.writeHead(200, { 'content-type': contentTypeValue, 'cache-control': 'public, max-age=86400' });
  res.end(value);
}
